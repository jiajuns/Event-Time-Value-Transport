...
## 22. router-development320：把逐头路由选择与 Formal190 分离

进一步审计确认，旧 development300 的 `internal30` 已用于 adapter best-step/checkpoint selection，不能再次充当 dual-provider router 选择集；其 30 个组也不可能满足 success/object 每侧 50 个独立 execution group 和 semantic cluster 的门。若改用 Formal190 完成 nested OOF 并冻结 route，则 Formal190 只能证明选择算法的 outer-OOF 行为，不能再被表述为固定 route 的独立正式验证。为了保留 Formal190 的一次性未见验证含义，新增了平行且完全不改写旧 v1 的 `scripts/preregister_smolvla_piper_schema6_routerdev320_v1.py`，详细合同见 `docs/ETSF_SmolVLA_Piper_schema6_routerdev320_preregistration_v1.md`。

该协议标签盲冻结 320 个 Piper-only semantic-reset/execution group、每组四个候选记账身份，只有一个物理 split `dual_provider_router_development`。canonical outer 五折每折 held out 64 个组，outer-train 为 256；每个 outer 内的 inner 五折最小 train scope 为 204。按公开 Source63 分支 success 率 Wilson 95% 下界 `0.0981819861` 和“四候选 IID”这一未验证的规划假设，25 个 inner scope 同时取得至少 50 个 success-positive group 的 union-bound 规划要求最小 inner scope 为 203，对应总量约 318，工程取 320。这个计算只解释规模，不保证真实 Piper 分布，更不保证 event、duration、recovery 或 object 支持；真实标签审计失败时必须禁用 head，不能补 seed、换 split 或复用 Formal190。

routerdev320 使用独立 seed namespace，并显式避开旧 development300 与 Source dense260 的已知候选范围。candidate/reset-selected 两阶段都要求外部签名证明它与完整 provider training closure（包括 train80 与用于选 checkpoint 的 internal30）、Formal190、evaluation400 在 requested/resolved/semantic/execution identity 上不相交。两个 provider 各五个 checkpoint 必须先对 label-blind input view 完成真实 forward 并冻结 raw prediction tensors，之后才允许单独 authority 打开 router targets。该预注册自身的 reset、collection、training、prediction、label-open、calibration、promotion、deployment 和性能声明权限全部为 false；当前仍没有真实 routerdev320 数据，也没有启动训练。
